...
- **Follow the process in `process.md`.** All work goes documents-first: feature description (`docs/features/`) → architecture (`docs/architecture/`) → knowledge-gap analysis (`docs/knowledge/`) → code → knowledge capture. `docs/knowledge/` models what David currently knows, one file per subject — read the relevant topic files before explaining things (don't re-teach what's marked solid; do teach what's marked new), and update them after each feature with what David learned.
